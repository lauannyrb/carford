...
def add_car():
    ...
    if color not in available_colors:
        return {
            'message': f"Invalid or missing color. Available colors: {', '.join(available_colors)}"
        }, 400
    ...
    if model not in available_models:
        return {
            'message': f"Invalid or missing model. Available models: {', '.join(available_models)}"
        }, 400
